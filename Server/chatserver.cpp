  ...
  while (udpSocket_->hasPendingDatagrams())
    {
      ...
      QHostAddress sender;
      quint16 sender_port;
      udpSocket_->readDatagram(data.data(), data.size());//, &sender, &sender_port);
      for (auto i = users_address_.begin(); i != users_address_.end(); ++i)
        {
          QUdpSocket *udp = *(i);
          //if (sender != udp->peerAddress())
            udpSocket_->writeDatagram(data, data.size(),udp->peerAddress(), udp->peerPort());
        }
    }
}
  ...
  if (it != users_.end())
    {
      ...
      //users_address_.erase(udp);
      // mb pair <id, udpsocket>?
    }
  rapidjson::Document d;
  d.SetObject();
  rapidjson::Document::AllocatorType& allocator = d.GetAllocator();
