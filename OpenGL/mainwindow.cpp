...
void MainWindow::draw_face()
{
    ...
    for (size_t i = 0; i < faces_.size(); ++i)
    {
        ...
        for (int j = 0; j < 3; ++j)
        {
            ...
            world_coords[j]  = verts_triangle;
        }
        point_3f n = (world_coords[2]-world_coords[0])^(world_coords[1]-world_coords[0]);
        n.normalize();
        float intensity = n*light_dir;
        if (intensity>0)
            triagle(screen_coords, intensity, zbuffer, tex_coords);
    }
    ui->label->setPixmap(QPixmap::fromImage(image_->mirrored(),Qt::AutoColor));
}

void MainWindow::triagle(point_3i screen_coords[], float intensity, int *zbuffer, point_2i tex_coords[])
{
    point_3i t0 = screen_coords[0];
    point_3i t1 = screen_coords[1];
    point_3i t2 = screen_coords[2];
    ...
    point_2i uv0 = tex_coords[0];
    point_2i uv1 = tex_coords[1];
    point_2i uv2 = tex_coords[2];
    if (uv0.y > uv1.y)
        std::swap(uv0, uv1);
    if (uv0.y > uv2.y)
        std::swap(uv0, uv2);
    ...
    for (int i = 0; i < total_height; ++i)
    {
        ...
        for (int j = A.x; j <= B.x; ++j)
        {
            ...
            {
                zbuffer[idx] = P.z;
                QRgb rgb_pix = texture_.pixel(uvP.x, uvP.y);
                QColor color_pix(rgb_pix);
                int r = color_pix.red();
                int g = color_pix.green();
                int b = color_pix.blue();
                image_->setPixel(P.x, P.y, qRgb(intensity*r, intensity*g, intensity*b));
            }
        }
    }
}
